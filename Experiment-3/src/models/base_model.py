    ...
    def train_generator(self, dataset, shuff_index, batch_size):
        ...
        while 1:
            for i in range(num_iters):
                ...
                elif self.batch_augment_fn is not None:
                    x, y = self.batch_augment_fn(tmp, dataset['y_train'][idx])
                else:
        ...
        while 1:
            for i in range(num_iters):
                ...
                if self.batch_format_fn is not None:
                    ...
                    x, y = self.batch_augment_fn(tmp, dataset['y_valid'][i*batch_size:(i+1)*batch_size])                    
                else:
